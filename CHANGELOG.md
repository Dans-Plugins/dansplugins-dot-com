...
- A drift guard (`__tests__/pageLandmarks.test.ts`) now walks `pages/` and fails if a page declares neither the `main` landmark nor an `<h1>`, in the style of the existing sitemap guard, so neither omission can recur silently.
