...
- The plugin catalogue now exists as a `plugins` table in `dpc-api` (`V15`), seeded from `pages/data/plugins.json` and served read-only at `GET /api/v1/plugins` and `GET /api/v1/plugins/{slug}`. This is the first half of moving the catalogue off a checked-in file; the site still renders from the file, and `__tests__/pluginCatalogue.test.ts` fails if the two disagree. The second half is #87. See `RESOURCE_HUB.md` (#272).
- `RESOURCE_HUB.md` documents the design of the resource hub the road map now tracks: what SpigotMC does that this site doesn't, which of those features are worth copying, and the decisions behind them — mirroring GitHub releases rather than hosting files, keeping reviews separate from discussion and from bug reports, and marking reviews from people whose servers actually run the plugin (#271).
