...
- The site now builds on Next.js 14.2.35, up from 12.2.2 — the version the sibling site already runs and the floor `@kingdom-community/community-site-kit` requires (`next >= 13`), so the shared component kit that exists to retire this site's duplicated components can now be adopted. React moves to 18.3.1, TypeScript to 5.4.5, and `eslint-config-next` to 14.2.35 alongside it; the app stays on the `pages/` router. The change that mattered is `components/NextLinkComposed.tsx`: under Next 12 `next/link` expected a child `<a>`, so the component wrapped one in `passHref`, and on Next 13+ the Link renders its own anchor — leaving that wrapper in place emitted `<a href="/guides"><a></a></a>`, invalid HTML that breaks styling and confuses assistive technology. The component now hands the ref and the anchor props to the Link itself. Its public shape (`component={NextLinkComposed} to="/path"`) is unchanged, so none of its twenty-odd call sites needed editing, and a new render test pins that exactly one anchor comes out. `swcMinify: false` leaves `next.config.js`, as `CONFIG.md` said it should once the dependency moved past v12: v13 made SWC minification the default and v15 removes the option outright.
...
- The home page's "Latest: vX.Y.Z" chips now come from the release mirror instead of GitHub. Every render asked GitHub for each catalogue plugin's latest release — sixteen unauthenticated calls against a documented budget of sixty an hour, which four page loads exhaust, after which every chip silently disappeared for the rest of the hour. `dpc-api` has mirrored those releases since 0.15.0, so the tags are read from it in a single request to a new `GET /api/v1/plugins/versions/latest`, alongside the bStats lookups rather than after them. Two consequences are worth naming: a chip now says what the resource page says, since both resolve "latest" as the newest *stable* release and fall back to the newest pre-release for a plugin that has only those — GitHub's `/releases/latest` answers `404` in that case, so such a plugin used to show a chip on its page and none on its card — and the home page no longer falls back to GitHub at all, so if `dpc-api` is unreachable the chips are absent rather than fetched a plugin at a time. That last case is written to the server log rather than passed over in silence, since one unanswered request now blanks every chip at once and nothing is left behind it to notice. `getLatestReleasesWithRateLimit` goes with the last caller that needed it; `getLatestRelease` stays as the resource page's single-plugin fallback (#299).
...
- `USER_GUIDE.md`'s walk-through of the home page now describes the two chips a plugin card carries — the bStats server count and the "Latest: vX.Y.Z" release tag — and says that each appears only when its figure is available, so a reader comparing two cards can tell why one shows a chip the other does not. The **Latest** chip's absence is the meaningful case: since the tag comes from the release mirror, a plugin that has published no release, or whose releases have not been mirrored yet, simply has none. Both figures were already documented for the plugin's own page, one step further in (#302).

### Fixed

- Server-side reads of `dpc-api` now work in the shipped Docker Compose stack. `NEXT_PUBLIC_API_URL` was one value doing two jobs — the address the browser calls and the address the Next.js server calls while rendering — and under Compose those are different hosts: the browser needs the published host port `http://localhost:45345`, which from inside the `dpc-website` container is that container's own loopback with nothing listening. Every server-rendered read therefore failed on a default `./up.sh`, silently, because each helper swallows its own error: a resource page showed no **Versions** section and no downloads chip, and the home page showed no "Latest: vX.Y.Z" chips at all. A second variable, `DPC_API_INTERNAL_URL`, now names the server's route to the API and is read by `getApiBaseUrl` only while `window` is absent, falling back to the public URL when unset — so a deployment whose API answers at one origin from both sides needs nothing new, while `compose.yml` sets it to `http://dpc-api:8080`. Being server-only it is never exposed to the browser and is read at runtime rather than inlined at build time, so changing it needs a restart rather than an image rebuild (#301).
