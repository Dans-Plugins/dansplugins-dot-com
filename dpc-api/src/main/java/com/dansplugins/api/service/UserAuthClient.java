...
/**
 * Client for the standalone UserAuth service
 * (https://github.com/Preponderous-Software/UserAuth).
 *
 * dpc-api delegates all credential handling to UserAuth: registration, login (JWT
 * issuance), per-request token validation (which also honours revocation), and
 * logout (token revocation). UserAuth is internal-only, so the browser talks to
 * dpc-api and dpc-api proxies to UserAuth. The base URL is configured via
 * {@code userauth.url} and defaults to {@code http://localhost:9998}.
 */
@Service
public class UserAuthClient {
     ...
     * UserAuth's 4xx status is propagated verbatim (e.g. 409 username taken); a 5xx
     * or connection failure surfaces as 503.
     */
    ...
    /**
     * Validate a bearer token against UserAuth {@code GET /session/validate}. Returns the
     * authenticated username when the token is valid (signature, expiry, and not revoked),
